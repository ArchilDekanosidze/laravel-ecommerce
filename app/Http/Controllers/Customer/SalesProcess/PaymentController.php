...
class PaymentController extends Controller
{
    ...
    public function copanDiscount(Request $request)
    {
        ...
        if ($copan != null) {
            if ($copan->user_id != null) {
                ...
                if ($copan == null) {
                    return redirect()->back()->withErrors(['copan' => [__('public.The discount code was entered incorrectly')]]);
                }
            }
            ...
            if ($order) {
                ...
                return redirect()->back()->with(['copan' => __('public.The discount code has been successfully applied')]);
            } else {
                return redirect()->back()->withErrors(['copan' => [__('public.The discount code was entered incorrectly')]]);
            }
        } else {
    ...
    public function paymentSubmit(Request $request, PaymentService $paymentService)
    {
        ...
        switch ($request->payment_type) {
            ...
            case '3':
                ...
                break;
            default:
                return redirect()->back()->withErrors(['error' => __('public.error')]);
        }
        ...
        return redirect()->route('customer.home')->with('success', __('public.Your order has been successfully placed'));
    ...
    public function paymentCallback(Order $order, OnlinePayment $onlinePayment, PaymentService $paymentService)
    {
        ...
        if ($result['success']) {
            ...
            return redirect()->route('customer.home')->with('success', __('public.Your payment has been successfully completed'));
        } else {
            $order->update(
                ['order_status' => 2]
            );
            return redirect()->route('customer.home')->with('danger', __('public.Your order encountered an error'));
        }
