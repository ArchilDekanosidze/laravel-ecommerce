...
class ProductController extends Controller
{
    public function product(Product $product)
    {
        ...
        return view('customer.markets.products.product', compact('product', 'relatedProducts'));
    }
    ...
    public function addRate(Product $product, Request $request)
    {
        ...
        if (Auth::check() && $productIds->count() > 0) {
            ...
            return back()->with('alert-section-success', __('public.Your score has been successfully registered'));
        } else {
            return back()->with('alert-section-error', __('public.You are not allowed to register points. You must buy the product first'));
        }
    }
