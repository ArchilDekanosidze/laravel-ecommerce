...
@section('head-tag')
<title>{{__('admin.edit banner')}}</title>
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item font-size-12 active" aria-current="page">{{__('admin.edit banner')}}</li>
    </ol>
</nav>
...
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section>
                <form action="{{ route('admin.content.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data" id="form">
                    @csrf
                    {{ method_field('put') }}
                    <section class="row">
                        ...
                        <section class="col-12 col-md-6 my-2">
                            <div class="form-group">
                                ...
                                <select name="status" id="" class="form-control form-control-sm" id="status">
                                    <option value="0" @if (old('status', $banner->status) == 0) selected @endif>{{__('admin.deactive')}}
                                    </option>
                                    <option value="1" @if (old('status', $banner->status) == 1) selected @endif>{{__('admin.active')}}
                                    </option>
                                </select>
                            </div>
                            @error('status')
                        ...
                        <section class="col-12">
                            <div class="form-group">
                                ...
                                <select name="position" id="" class="form-control form-control-sm">
                                    ...
                                    <option value="{{ $key }}" @if(old('position', $banner->position) == $key) selected
                                        @endif>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
