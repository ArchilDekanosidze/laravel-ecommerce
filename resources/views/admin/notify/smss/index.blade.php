...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item font-size-12 active" aria-current="page">{{__('admin.SMS')}}</li>
    </ol>
</nav>
    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                <a href="{{ route('admin.notify.smss.create') }}" class="btn btn-info btn-sm">{{__('admin.create SMS')}}</a>
            ...
            <section class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>{{__('admin.SMS subject')}}</th>
                            <th>{{__('admin.SMS body')}}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <label>
                                    <input id="{{ $single_sms->id }}" onchange="changeStatus({{ $single_sms->id }})"
                                        data-url="{{ route('admin.notify.smss.status', $single_sms->id) }}"
                            ...
                            <td class="width-16-rem text-left">
                                <a href="{{ route('admin.notify.smss.edit', $single_sms->id) }}"
                                    class="btn btn-info btn-sm"><i class="fa fa-edit"></i> {{__('admin.edit')}}</a>
                                <form class="d-inline"
                                    action="{{ route('admin.notify.smss.destroy', $single_sms->id) }}" method="post">
                                    @csrf
                                    {{ method_field('delete') }}
        ...
        success: function(response) {
            if (response.status) {
                if (response.checked) {
                    ...
                    successToast("{{__('admin.SMS activated successfully')}}")
                } else {
                    element.prop('checked', false);
                    successToast("{{__('admin.SMS deactivated successfully')}}")
                }
            } else {
