...
@section('head-tag')
<title>{{__('admin.edit email')}}</title>
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item font-size-12 active" aria-current="page">{{__('admin.edit email')}}</li>
    </ol>
</nav>
...
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section>
                <form action="{{ route('admin.notify.emails.update', $email->id) }}" method="post">
                    @csrf
                    {{ method_field('put') }}
                    <section class="row">
                        ...
                        <section class="col-12">
                            <div class="form-group">
                                ...
                                <select name="status" id="" class="form-control form-control-sm" id="status">
                                    <option value="0" @if (old('status', $email->status) == 0) selected @endif>{{__('admin.deactive')}}
                                    </option>
                                    <option value="1" @if (old('status', $email->status) == 1) selected @endif>{{__('admin.active')}}
                                    </option>
                                </select>
                            </div>
                            @error('status')
