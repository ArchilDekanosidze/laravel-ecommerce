...
@section('head-tag')
<title>{{__('admin.edit coupon')}}</title>
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item font-size-12 active" aria-current="page">{{__('admin.edit coupon')}}</li>
    </ol>
</nav>
...
<section class="row">
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section>
                <form action="{{ route('admin.market.discounts.copan.update', $copan->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <section class="row">
                        ...
                        <section class="col-12 col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="code" value="{{ old('code', $copan->code) }}"
                        ...
                        <section class="col-12 col-md-6">
                            <div class="form-group">
                                ...
                                <select name="type" id="type" class="form-control form-control-sm">
                                    <option value="0" @if(old('type', $copan->type) == 0) selected @endif>{{__('admin.public')}}</option>
                                    <option value="1" @if(old('type', $copan->type) == 1) selected @endif>{{__('admin.private')}}</option>
                                </select>
                            </div>
                        </section>
                        <section class="col-12 col-md-6">
                        ...
                        <section class="col-12 col-md-6">
                            <div class="form-group">
                                ...
                                <select name="amount_type" id="amount_type" class="form-control form-control-sm">
                                    <option value="0" @if(old('amount_type', $copan->amount_type) == 0) selected
                                        @endif>{{__('admin.percentage')}}</option>
                                    <option value="1" @if(old('amount_type', $copan->amount_type) == 1) selected
                                        @endif>{{__('admin.numerical')}}</option>
                                </select>
                            </div>
                            @error('amount_type')
                            <span class="alert_required bg-danger text-white p-1 rounded" role="alert">
                        ...
                        <section class="col-12">
                            <div class="form-group">
                                ...
                                <select name="status" id="" class="form-control form-control-sm" id="status">
                                    <option value="0" @if(old('status', $copan->status) == 0) selected @endif>{{__('admin.deactive')}}
                                    </option>
                                    <option value="1" @if(old('status', $copan->status)) == 1) selected @endif>{{__('admin.active')}}
                                    </option>
                                </select>
                            </div>
                            @error('status')
