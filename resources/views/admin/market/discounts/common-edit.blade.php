...
@section('head-tag')
<title>{{__('admin.edit common discount')}}</title>
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item font-size-12"> <a href="#">{{__('admin.common discount')}}</a></li>
        <li class="breadcrumb-item font-size-12 active" aria-current="page">{{__('admin.edit common discount')}}</li>
    </ol>
</nav>
    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h5>
                {{__('admin.edit common discount')}}
                </h5>
            </section>

            <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                <a href="{{ route('admin.market.discounts.commonDiscount') }}" class="btn btn-info btn-sm">{{__('admin.return')}}</a>
            </section>

            <section>
                <form action="{{ route('admin.market.discounts.commonDiscount.update', $commonDiscount->id) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <section class="row">
                        ...
                        <section class="col-12 col-md-6">
                            <div class="form-group">
                                <label for="">{{__('admin.minimal order amount')}}</label>
                        ...
                        <section class="col-12 col-md-6">
                            <div class="form-group">
                                <label for="">{{__('admin.occasion')}}</label>
                        ...
                        <section class="col-12">
                            <div class="form-group">
                                ...
                                <select name="status" id="" class="form-control form-control-sm" id="status">
                                    <option value="0" @if (old('status', $commonDiscount->status) == 0) selected
                                        @endif>{{__('admin.deactive')}}</option>
                                    <option value="1" @if (old('status', $commonDiscount->status) == 1) selected
