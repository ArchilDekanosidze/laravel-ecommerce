...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item font-size-12"> <a href="#">{{__('admin.roperty value')}}</a></li>
        <li class="breadcrumb-item font-size-12 active" aria-current="page">{{__('admin.create property value')}}</li>
    </ol>
</nav>
    ...
    <section class="col-12">
        <section class="main-body-container">
            <section class="main-body-container-header">
                <h5>
                {{__('admin.create property value')}}
                </h5>
            </section>
            ...
            <section>
                <form action="{{ route('admin.market.properties.values.store', $categoryAttribute->id) }}"
                    method="POST">
                    @csrf
                    <section class="row">
                        ...
                        <section class="col-12">
                            <div class="form-group">
                                ...
                                <select name="type" id="" class="form-control form-control-sm" id="type">
                                    <option value="0" @if(old('type')==0) selected @endif>{{__('admin.single')}}</option>
                                    <option value="1" @if(old('type')==1) selected @endif>{{__('admin.multiple')}}</option>
                                </select>
                            </div>
                            @error('type')
                            <span class="alert_required bg-danger text-white p-1 rounded" role="alert">
