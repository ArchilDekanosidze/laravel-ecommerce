                    ...
                    <section class="col-md-5">
                        ...
                        <section class="content-wrapper bg-white p-3 rounded-2 mb-4">
                            ...
                            <section class="product-info">
                                <form id="add_to_cart"
                                    action="{{ route('customer.sales-process.carts.add-to-cart', $product) }}"
                                    ...
                                    @if($colors->count() != 0)
                                    <p><span>{{__('public.choosed color')}} : <span id="selected_color_name"> {{
                                    ...
                                    <p>
                                        ...
                                        <i class="fa fa-store-alt cart-product-selected-store me-1"></i> <span>{{__('public.Items remaining in store')}}</span>
                                        @else
                                        <i class="fa fa-store-alt cart-product-selected-store me-1"></i> <span>{{__('public.product unavailable')}}</span>
                                        @endif
                                    </p>
                                    ...
                                    <div class="d-flex">
                                        ...
                                        <section class="product-add-to-favorite position-relative" style="top: 0">
                                            <button type="button" class="btn btn-light btn-sm text-decoration-none"
                                                data-url="{{ route('customer.markets.add-to-favorite', $product) }}"
                                        ...
                                        <section class="product-add-to-compare position-relative" style="top: 0">
                                            <button type="button" class="btn btn-light btn-sm text-decoration-none"
                                                data-url="{{ route('customer.markets.add-to-compare', $product) }}"
                                                data-bs-toggle="tooltip" data-bs-placement="left"
                                    ...
                                    <p class="mb-3 mt-5">
                                        <i class="fa fa-info-circle me-1"></i>{{__('public.lorem')}} </p>
                            </section>
                        </section>
                    ...
                    <section class="col-md-3">
                        <section class="content-wrapper bg-white p-3 rounded-2 cart-total-price">
                            ...
                            <section class="d-flex justify-content-between align-items-center">
                                ...
                                <p class="text-danger fw-bolder" id="product-discount-price"
                                    ...
                                    {{ priceFormat($product->price * ($amazingSale->percentage / 100) ) }} <span
                                        class="small">{{__('public.Dollar')}}</span></p>
                            </section>
                            @endif
                            ...
                            <section class="d-flex justify-content-end align-items-center">
                                <p class="fw-bolder"><span id="final-price"></span> <span class="small">{{__('public.Dollar')}}</span></p>
                            </section>

                            <section class="">
                                @if($product->marketable_number > 0)
                                <button id="next-level" class="btn btn-danger d-block w-100"
                                    onclick="document.getElementById('add_to_cart').submit();">{{__('public.add to cart')}}</button>
                                @else
                                <button id="next-level" class="btn btn-secondary disabled d-block">{{__('public.product unavailable')}}</button>
                                @endif
                            </section>
                            </form>
                            ...
                            <section class="d-flex justify-content-between align-items-center">
                                <h2 class="content-header-title">
                                    <span class="me-2"><a class="text-decoration-none text-dark"
                                            href="#introduction">{{__('public.description')}}</a></span>
                                    <span class="me-2"><a class="text-decoration-none text-dark"
                                            href="#features">{{__('public.features')}}</a></span>
                                    <span class="me-2"><a class="text-decoration-none text-dark"
                                            href="#comments">{{__('public.comments')}}</a></span>
                                    <span class="me-2"><a class="text-decoration-none text-dark" href="#rating">
                                            {{__('public.rates')}}</a></span>
                                </h2>
                                <section class="content-header-link">
                                    <!--<a href="#">View all</a>-->
                                </section>
                        ...
                        <section class="product-comments mb-4">
                            ...
                            <section class="comment-add-wrapper">
                                <button class="comment-add-button" type="button" data-bs-toggle="modal"
                                    data-bs-target="#add-comment"><i class="fa fa-plus"></i> {{__('public.add comment')}}</button>
                                        ...
                                        <section class="modal-content">
                                            <section class="modal-header">
                                                <h5 class="modal-title" id="add-comment-label"><i
                                                        class="fa fa-plus"></i> {{__('public.add comment')}}</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </section>
                                            @guest
                                            <section class="modal-body">
                                                <p>{{__('public.dear user, you need to login to add comment')}} </p>
                                                <p>{{__('public.login - register link')}}
                                                    <a href="{{ route('auth.customers.login-register-form') }}"{{__('public.click')}}</a>
                                                </p>
                                            </section>
                                            @endguest
                                            @auth
                                            ...
                                            <section class="modal-body">
                                                <form class="row"
                                                    action="{{ route('customer.markets.add-comment', $product) }}"
                                                    method="post">
                                                    @csrf
                                                    <section class="col-12 mb-2">
                                                        <label for="comment" class="form-label mb-1">{{__('public.your comment')}}</label>
                                                        <textarea class="form-control form-control-sm" id="comment"
                                                            placeholder="{{__('public.your comment')}} ..." rows="4"
                                                            name="body"></textarea>
                                                    </section>

                                            </section>
                                            <section class="modal-footer py-1">
                                                <button type="submit" class="btn btn-sm btn-primary">{{__('public.add comment')}}</button>
                        ...
                        <section class="product-rating mb-4">
                            ...
                            <div class="container">
                                <h5 class="text-danger">
                                    {{__('public.add your rate to this product')}}
                                </h5>
                                <form
                                    class="starrating risingstar d-flex justify-content-end flex-row-reverse align-items-center"
                                    action="{{ route('customer.markets.add-rate', $product) }}" method="post">
                                    @csrf
                                    <div class="mx-3">
                                ...
                                <h6>
                                     {{__('public.average rate is :')}} : {{ number_format($product->ratingsAvg(), 1, '.') ?? "__('public.this is the first rate')" }}
                                </h6>
                                <h6>
                                    {{__('public.number of user that rated this product')}} : {{ $product->ratingsCount() ?? "__('public.this is the first rate')" }}
                                </h6>
                            </div>
...
<section class="position-fixed p-4 flex-row-reverse"
    ...
    <div class="toast" data-delay="7000" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-header">
            <strong class="me-auto">{{__('public.market')}}</strong>
            <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
        </div>
        <div class="toast-body">
            <strong class="ml-auto">
                {{__('public.you need to login to ad to favorites')}}
                <br>
                <a href="{{ route('auth.customers.login-register-form') }}" class="text-dark">
                    {{__('public.login/register')}}
                </a>
            </strong>
        </div>
    </div>
